...
このことを、RStudio では「編む」という意味の knit と言います。
...
最近は、ブラウザがポップアップを拒否することがあります。その場合は、以下のような警告が出ますが、ファイルをみていただくと HTML ファイルができていることが確認できます。
...
* R は、統計やグラフ作成に特化したコンピュータ言語であることを学びました。
* RStudio は、R を快適に使うための統合開発環境 (IDE) です。
* 記録は Rmd 形式で保存し、なるべく日本語の使用は避けることを学びました。
